"""Ximbra Ingestor — ingesta meteorológica periódica desde Open-Meteo hacia PostgreSQL."""
import logging
import os
import signal
import time
...
from app.application.ingest import IngestWeatherUseCase
...
def stop(sig, frame):
    global running
    logger.info("Señal recibida — deteniendo ingestor...")
    running = False
...
def main():
    ...
    use_case = IngestWeatherUseCase(db)

    logger.info(f"Ingestor iniciado — ciclo cada {CYCLE_INTERVAL}s")
